/*=====================================================================+
  ...
  -- 人力信息表
-- drop table if exists dwm.dwd_th_ffm_staff_dayV3;
-- create table dwm.dwd_th_ffm_staff_dayV3 as
# delete from dwm.dwd_th_ffm_staff_dayV3 where stat_date >= date_sub(date(now() + interval -1 hour),interval 60 day);
# insert into dwm.dwd_th_ffm_staff_dayV3
SELECT
    人员信息 staff_info_id
    ,统计日期 stat_date
    ,一级部门 dept_1_name
    ...
    --    ,应出勤-请假-旷工 出勤
    -- ,未出勤
    ,if(实际应出勤=0, 0, 请假) leave_days -- updateby 王昱棋 20250117 这里会设计提成计算，需要同步修改，针对有些人在节假日进行请假的情况。
    -- ,请假时段
        ...
        (
            ...
            (
                SELECT
                    ...
                    --     end 仓库
                    ,case   when ss2.name='AGV Warehouse' then 'AGV'
                            when ss2.name='BPL2-Bangphli Return Warehouse' then 'BPL-Return'
                            when ss2.name='BPL3-Bangphli Live Stream Warehouse' then 'BPL3'
                            when ss2.name='BST- Bang Sao Thong warehouse' then 'BST'
                            when ss2.name='BKK-WH-LAS2 E-Commerce Warehouse' then 'LAS'
                            end 仓库
                ...
                LEFT JOIN `fle_staging`.`sys_store` ss2 ON ss2.`id`=hst.`store_id` -- 20241211 addby 王昱棋 取历史的部门（仓库）
                left join
                (
                    select
                        申请日期
                    ...
                    (
                        ...
                        and ho.date_at >= date_sub(date(now() + interval -1 hour),interval 60 day)
                        -- and ho.staff_id='609471'
                    ) ot1
                    group by 1,2
                ) ot on ad.`staff_info_id` = ot.员工ID and ad.stat_date = ot.申请日期
                WHERE sd.`一级部门`='Thailand Fulfillment'
                    AND ad.`stat_date`>= date_sub(date(now() + interval -1 hour),interval 60 day)
                    AND ad.`stat_date`>='2023-12-01'
            ) ad
        ) ad
    ) ad
) ad
where 人员信息 IN ('606321' ,'661237','32310','672950','78178','636585')
AND 统计日期 ='2025-06-24'
;
