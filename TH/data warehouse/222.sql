SELECT
    ...
    ,LEFT(2Cdeadline,10) 日期
     ,outtimetype
#      ,delivery_sn
    ,week(date(2Cdeadline) + interval 1 day) 周
#     ,count(if(outtimetype in ('outbound intime'), delivery_sn, null)) 及时完成单量
#     ,count(if(outtimetype in ('outbound intime', 'nooutbound intime'), delivery_sn, null)) 应完成单量
    ,count(delivery_sn) 创建单量
#     ,count(if(outtimetype in ('outbound intime'), delivery_sn, null)) / count(if(outtimetype in ('outbound intime', 'nooutbound intime'), delivery_sn, null)) 及时率
FROM dwm.dwd_th_ffm_outbound_dayV2
where date(2Cdeadline)>='2025-06-01'
and is_visible=1
and type='B2C'
# and outtimetype='系统商品报缺'
group by 1,2,3,4,5
order by 3,2;
    ...
    (
        ...
        (
            SELECT
                ...
                ,goods_num
                ,warehouse_id
                ,case when warehouse_id in (36, 78)   then 'AGV'
                    when warehouse_id=33  then 'BPL-Return'
                    when warehouse_id in (18,82)  then 'BST'
                    when warehouse_id IN (39)    then 'LAS'
                     end warehouse_name
                ,case when warehouse_id in (36)   then 'AGV'
                    when warehouse_id in (78)   then 'AutoWarehouse-2'
                    when warehouse_id=33  then 'BPL-Return'
                    when warehouse_id=18  then 'BST'
                    when warehouse_id=82  then 'BST-FMCG'
                    when warehouse_id IN (39)    then 'LAS'
                    end warehouse_name2
                ,do.platform_source
...
(
    ...
    where 1=1
    and dt >= date_sub(date(now() + interval -1 hour),interval 30 day)
    group by 1,2,3
) dt
left join
(
    ...
    group by 1,2
) out on dt.warehouse_detailname = out.warehouse_detailname and dt.dt = out.dt
